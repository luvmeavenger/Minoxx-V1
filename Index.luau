...
local Settings = {
    ...
    TeamCheck = false,
    FOV = 120,
    Accent = Color3.fromRGB(138, 43, 226) -- Purple Theme
}
...
local function UpdateRadar()
    for _, v in pairs(Indicators) do v:Destroy() end
    ...
    for _, p in pairs(Players:GetPlayers()) do
        if p ~= LocalPlayer and p.Character and p.Character:FindFirstChild("HumanoidRootPart") then
            ...
            local rel = Camera.CFrame:PointToObjectSpace(p.Character.HumanoidRootPart.Position)
            local angle = math.atan2(rel.X, rel.Z)
            
            local dot = Instance.new("Frame", FOVCircle)
            dot.Size = UDim2.new(0, 4, 0, 4)
            dot.BackgroundColor3 = Settings.Accent
            dot.Position = UDim2.new(0.5 + (math.sin(angle) * 0.48), 0, 0.5 - (math.cos(angle) * 0.48), 0)
...
local function AddToggle(text, prop)
    ...
    b.MouseButton1Click:Connect(function()
        ...
        b.BackgroundColor3 = Settings[prop] and Color3.fromRGB(30, 30, 35) or Color3.fromRGB(20, 20, 23)
